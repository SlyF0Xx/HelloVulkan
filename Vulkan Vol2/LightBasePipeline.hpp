...
public:
	...
	LightBasePipeline(string vertShader, string fragShader,
	...
	virtual ~LightBasePipeline();
};

template<class Vertex, class Model>
inline VkRenderPass LightBasePipeline<Vertex, Model>::GetRenderPass()
{
	return RenderPass;
}

template<class Vertex, class Model>
inline LightBasePipeline<Vertex, Model>::LightBasePipeline(
	string vertShader, string fragShader, vector<VkVertexInputBindingDescription> VertexInputDesc, vector<VkVertexInputAttributeDescription> VertexInputAttrDesc,
	vector<VkAttachmentDescription> Attachments, Logger * logger, LogicDeviceWrapper Device, vector<VkDescriptorSetLayout> DescriptorSets):
		AbstractPipelineWrapper<Vertex, Model, 1>(logger, Device), DescriptorsLay(DescriptorSets)
{
	...
	DepthStensilInfo.depthCompareOp = VK_COMPARE_OP_LESS_OR_EQUAL;// VK_COMPARE_OP_GREATER_OR_EQUAL;
