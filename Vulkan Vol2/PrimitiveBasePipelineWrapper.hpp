		...
		:AbstractPipelineWrapper<Vertex, Model, 2>(logger, device), DescriptorsLay(DescriptorSets)
{
	...
	InputAssemblyInfo.primitiveRestartEnable = VK_FALSE; //ÐÀçîáðàòüñÿ!!
	...
	RasterizationInfo.rasterizerDiscardEnable = VK_FALSE; //ÍÅ ÒÐÎÃÀÒÜ ÍÈÊÎÃÄÀ Â ÆÈÇÍÈ!!! ÂÛÊÈÄÛÂÀÅÒ ÈÇÎÁÐÀÆÅÍÈÅ ÏÎÑËÅ ÐÀÑÒÅÐÈÇÀÖÈÈ
	...
	RasterizationInfo.depthBiasEnable = VK_FALSE; //Íå ðàçîáðàëñÿ
	RasterizationInfo.depthBiasConstantFactor = 0.0;
	RasterizationInfo.depthBiasClamp = 0.0; //Íå ðàçîáðàëñÿ
	RasterizationInfo.depthBiasSlopeFactor = 0.0; //Íå ðàçîáðàëñÿ
	RasterizationInfo.lineWidth = 1.0;


	VkSampleCountFlagBits Sample = VK_SAMPLE_COUNT_1_BIT;
	//Ãîâîðÿò, ýòî ïðî anti-aliasing
	...
	DepthStensilInfo.depthBoundsTestEnable = VK_FALSE; //Åñëè VK_TRUE, òî äåëàåò îòäåëüíûé z òåñò íà minDepthBounds è maxDepthBounds
	DepthStensilInfo.stencilTestEnable = VK_FALSE; // Ðàçîáðàòüñÿ è äîáàâèòü - êëàññíàÿ, âðîäå, øòóêà
	...
	//ColorBlendInfo.logicOpEnable = VK_FALSE; //Íàñêîëüêî ÿ ïîíÿë, ìîæíî ñäåëàòü îïåðàöèþ ïî ñðàâíåíèþ ñ òåêóùèì çíà÷åíèåì â êàäðå
	...
	Subpasses[0].flags = 0; // Âîçìîæíû âàðèàíòû
